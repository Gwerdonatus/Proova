...
function PrimaryAction({
  ...
  return (
    <a href={href} target="_blank" rel="noreferrer" className="w-full sm:w-auto">
      <button
        className="
          inline-flex h-12 w-full min-w-[220px] items-center justify-center whitespace-nowrap rounded-2xl
...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-black/8 bg-white p-4 shadow-[0_10px_24px_rgba(16,24,40,0.04)]">
      ...
      <div className="mt-1 text-sm font-semibold text-app-ink">{value}</div>
    </div>
  );
}
...
function getReferenceLabel(searchParams: SearchParams) {
  if (searchParams.provider === "paystack" && searchParams.reference) {
    return `Reference: ${searchParams.reference}`;
  }

  if (searchParams.provider === "paddle" && searchParams.transaction_id) {
    return `Transaction: ${searchParams.transaction_id}`;
  }

  return "Your payment details have been recorded.";
}
...
export default async function FounderSuccessPage({
  ...
  const referenceLabel = getReferenceLabel(params);
          ...
          <div className="p-6 sm:p-8 lg:p-10">
            ...
            <div className="mt-7 max-w-2xl">
              ...
              <p className="mt-3 text-sm leading-7 text-app-muted sm:text-base">
                Your Founder spot is now locked in. You’ll get early access, priority onboarding, and direct support as we roll
                out the private beta.
              </p>
            </div>
            ...
            <section className="mt-7 rounded-[28px] border border-black/8 bg-[#fbfbfc] p-5 shadow-[0_12px_30px_rgba(16,24,40,0.04)] sm:p-6">
              <div className="max-w-2xl">
                ...
                <p className="mt-2 text-sm leading-7 text-app-muted">
                  Book a quick onboarding call so we can map your sales flow — website, WhatsApp, transfers, or checkout —
                  and make sure your attribution setup is correct from day one.
                </p>
              </div>
              ...
              <div className="mt-5 rounded-2xl border border-black/8 bg-white p-4 shadow-[0_8px_20px_rgba(16,24,40,0.03)]">
                <div className="text-sm font-semibold text-app-ink">What we’ll do on the call</div>
            ...
            <section className="mt-5 rounded-[28px] border border-black/8 bg-white p-5 shadow-[0_12px_30px_rgba(16,24,40,0.04)] sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-black/8 bg-[#fbfbfc] p-4">
                  <div className="text-xs font-semibold text-app-muted">Next step</div>
                  <div className="mt-1 text-sm font-semibold text-app-ink">Founder welcome message</div>
                  <div className="mt-1 text-sm text-app-muted">You’ll get onboarding details and setup instructions</div>
                </div>

                <div className="rounded-2xl border border-black/8 bg-[#fbfbfc] p-4">
                  <div className="text-xs font-semibold text-app-muted">Private beta</div>
                  <div className="mt-1 text-sm font-semibold text-app-ink">Cohort-based rollout</div>
                  <div className="mt-1 text-sm text-app-muted">You’ll be part of the first group</div>
                </div>

                <div className="rounded-2xl border border-black/8 bg-[#fbfbfc] p-4">
                  <div className="text-xs font-semibold text-app-muted">Protection</div>
                  <div className="mt-1 text-sm font-semibold text-app-ink">30-day refund coverage</div>
                  <div className="mt-1 text-sm text-app-muted">Covered under the Founder refund promise</div>
                </div>
              </div>
